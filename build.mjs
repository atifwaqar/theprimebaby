// build.mjs — Build-time static exporter using LiquidJS (no Shopify backend).
// Generates dist/index.html and copies assets.
...
// Generate candidate paths for sections/snippets/layout
function candidatePaths(filepath, ext) {
  ...
  const isSnippet = withExt.startsWith("snippets/");
  const isSection = withExt.startsWith("sections/");
  const isLayout = withExt.startsWith("layout/");
...
const engine = new Liquid({
  ...
  fs: {
    ...
    readFile: async (filepath) => {
      for (const candidate of candidatePaths(filepath, ".liquid")) {
        try {
          const abs = path.join(THEME_DIR, candidate);
          const src = await fs.readFile(abs, "utf8");
          return preprocessLiquid(src);
        } catch {}
      }
      // Stub fallback
