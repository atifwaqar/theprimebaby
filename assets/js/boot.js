// Minimal LiquidJS runtime for Shopify themes on static hosting (GitHub Pages).
...
// Normalize Dawn-style blocks (array or object map)
function normBlocks(b){
  if(!b) return [];
  if(Array.isArray(b)) return b.map(x => ({ id:x.id || x.type, type:x.type, settings:x.settings || {} }));
  return Object.entries(b).map(([id,x]) => ({ id, type:x?.type, settings:(x && x.settings) || {} }));
}

// Minimal product adapter so snippets see Shopify-like fields
function adaptProduct(p){
  const price = Number(p.price ?? 0), cap = Number(p.compare_at_price ?? 0);
...
(async () => {
  ...
  // IMPORTANT: add an FS adapter so {% render %}/{% include %} files get preprocessed too
  const engine = new Liquid({
    ...
    fs: {
      exists: async (fp) => {
        try { const r = await fetch(`templates/${fp}`, { method: "HEAD", cache: "no-store" }); return r.ok; }
        catch { return false; }
      },
      readFile: async (fp) => pre(await t(`templates/${fp}`)),
      resolve: (root, file, ext) => {
        const name = file.endsWith(ext) ? file : file + ext;
        // default to snippets/ when no path provided
  ...
  // Data
  const settings = await j("data/settings.json").catch(()=>({ shop_name:"Shop", shop_description:"", money_format:"${{amount}}" }));
  ...
  for (const id of order) {
    ...
    const raw = await t(`templates/sections/${sec.type}.liquid`).catch(()=> "");
    if (!raw) continue;
    const html = await engine.parseAndRender(pre(raw), {
      ...
      section:{ id, type:sec.type, settings:sec.settings || {}, blocks:normBlocks(sec.blocks) }
    });
    body += html + "\n";
  }

  // Wrap with layout so theme CSS/JS load
  let layout = ""; try { layout = pre(await t("templates/layout/theme.liquid")); } catch {}
  const outer = layout
    ? await engine.parseAndRender(layout, { content_for_layout: body, shop:{ name:settings.shop_name, description:settings.shop_description, money_format:settings.money_format }, settings })
    : body;

  document.getElementById("app").innerHTML = outer;
})();
